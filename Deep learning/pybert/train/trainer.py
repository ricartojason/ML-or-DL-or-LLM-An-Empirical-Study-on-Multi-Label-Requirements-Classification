...
class Trainer(object):
    ...
    def valid_epoch(self,data):
        ...
        for step, batch in enumerate(data):
            ...
            with torch.no_grad():
                ...
                logits = self.model(input_ids, segment_ids,input_mask).to(self.device)
    ...
    # 加入标签平滑的二元交叉熵损失函数
    def label_smoothing_crossentropy(self,logits, target, epsilon=0.1, num_classes=4):
        smooth_value = 1-epsilon
        epsilon_value = epsilon / (num_classes - 1)
        #为每个样本创建一个平滑的目标向量
        # 新张量的形状（shape）与logits张量相同，且所有元素都被填充为smooth_value
        smooth_targets=torch.full_like(logits, smooth_value)
        # smooth_targets = torch.zeros_like(logits)

        for i ,target in enumerate(target):
            for class_index in range(num_classes):
                smooth_targets[i, class_index] = smooth_value if target[class_index] == 1 else epsilon_value
    ...
    def train_epoch(self,data):
        ...
        for step,  batch in enumerate(data):
            ...
            input_ids, input_mask, segment_ids, label_ids = batch
            logits = self.model(input_ids, segment_ids,input_mask).to(self.device)
            # mooth_target = self.label_smoothing_crossentropy(logits, label_ids, epsilon=0.1, num_classes=4)
        ...
        print("\n------------- train result --------------")
        # epoch metric
        # cat将75个list沿着第一个维度拼接在一起,变成[4800,5]的tensor
